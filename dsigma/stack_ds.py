...
__all__ = ["form_delta_sigma", "stack_delta_sigma", "stack_with_mask",
           "batch_delta_sigma", "batch_dsig_cov", "cov_with_mask"]
...
def cov_with_mask(lens_ds, lens_data, rand_ds, rand_data, lens_mask,
                  boost_factor=False, selection_bias=False, weight_field='weight',
                  rand_zweight_nbins=10, n_jobs=4, n_boots=1000,
                  boxsize=1, trunc=0.2, same_weight_rand=True):
    """Simple wrapper to generate DSigma profile with bootstrap resampling and
    covariance.
    """
    # Select a subsample of lenses if mask is available
    if lens_mask is not None:
    ...
    dsig_all, dsig_boot, dsig_jk = cov.get_bootstrap_samples_dsigma(
        lens_ds_use, rand_ds, lens_data_use, rand_data,
        n_boots=n_boots, n_jobs=n_jobs,
        z_bins=rand_zweight_nbins, selection_bias=selection_bias,
        weight_field=weight_field, use_boost=boost_factor,
        same_weight_rand=same_weight_rand)

    # Get the correlation and covariance matrix
    cor_org, cor_trunc, cov_trunc = fu.smooth_cov(
        dsig_boot, boxsize=boxsize, trunc=trunc)

    return {'dsig_all': dsig_all, 'dsig_boot': dsig_boot, 'dsig_jk': dsig_jk,
            'cor_org': cor_org, 'cor_trunc': cor_trunc, 'cov_trunc': cov_trunc}


def batch_delta_sigma(lens_pre, lens_data, mask_list,
...
def batch_dsig_cov(lens_pre, lens_data, mask_list,
                   rand_pre=None, rand_data=None, pickle_output=None,
                   njackknife_fields=31, boost_factor=False, selection_bias=False,
                   weight_field='weight', rand_zweight_nbins=10, same_weight_rand=True,
    ...
    single_stack = partial(cov_with_mask,
                           lens_ds=lens_ds, lens_data=lens_data,
                           rand_ds=rand_ds, rand_data=rand_data,
                           boost_factor=boost_factor,
                           selection_bias=selection_bias,
                           ...
                           rand_zweight_nbins=rand_zweight_nbins,
                           same_weight_rand=same_weight_rand,
                           n_jobs=n_jobs, n_boots=n_boots,
                           boxsize=boxsize, trunc=trunc)
